...
public static class JsonTools
{
    ...
    public static async Task WriteJsonToFileAsync(
        [Description("The run id used to collerate outputs into a single run context")]
        string runId,
        [Description("The file name for the json")]
        string fileName,
        [Description("The json content to be stored")]
        string json)
    {
    ...
    public static async Task<string> ReadJsonFromFileAsync(
        [Description("he run id used to collerate outputs into a single run context")]
        string runId,
        [Description("The file name of the json")]
        string fileName)
    {
        var path = Path.Combine($"/home/test/{runId}", fileName);
    ...
    [Description(
        "Lists all already created json files for a given run id. Returns a comma separated list with all file names.")]
    public static string ListAllJsonFilesAsync(
        [Description("he run id used to collerate outputs into a single run context")]
        string runId)
    {
        if (!Directory.Exists($"/home/test/{runId}")) return "";
